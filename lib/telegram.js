...
class TelegramNotifier {
    ...
    async sendMessage(text) {
        if (!this.bot || !this.chatId) {
            logger.debug(`Telegram disabled - would send: ${text}`);
            return;
        }

        try {
            await this.bot.sendMessage(this.chatId, text);
            logger.debug(`Telegram message sent: ${text}`);
        } catch (error) {
    ...
    async notifyHCaptchaDetected(groupId, dvsaUsername, sitekey, isReservation = false) {
        ...
        if (!isReservation) {
            logger.debug(`Skipping non-reservation hCaptcha alert for ${groupId}/${dvsaUsername}`);
            return;
        }
