...
class DiscordNotifier {
    constructor(webhookUrl) {
        ...
        if (this.isEnabled) {
            ...
            logger.info('⚠️ Discord webhook URL not provided - Discord notifications disabled');
        }
    }

    async sendMessage(message, title = null) {
        if (!this.isEnabled) {
            logger.debug('Discord notifier disabled - skipping message');
            return;
        }
    ...
    async notifyHCaptchaDetected(groupId, dvsaUsername, sitekey, url, isReservation = false) {
        if (!isReservation) {
            logger.debug('Discord: Skipping hCaptcha alert for non-reservation page');
            return;
        }
